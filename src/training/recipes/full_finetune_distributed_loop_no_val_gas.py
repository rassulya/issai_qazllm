...
class FullFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def save_checkpoint(
        ...
        if self._is_rank_zero:
            ...
            print(f"saved index:{self.sub_index}  cp:{self.global_step}")
    ...
    def sub_train(self,curr_epoch,world_size,rank):
        ...
        running_loss = 0
        num_tokens = 0
        self.checkpoint_saved = False # Using it to save checkpoint based on the Gradient Accumulation Step.
        ...
        for idx, batch in enumerate(self._dataloader if self.resource_falg else self.back_dataloader):
            ...
            # TODO: A condition to save checkpoint added need to test it.
            if self.global_step % self.save_per_step==0 and self.global_step != 0 and not self.checkpoint_saved:
            ...
            for j in range(_all_range_len):
                ...
                if self.resource_falg:
                    ...
                    if self._sampler and self._dataloader:
                        ...
                        sub_range = list(range(i,min(i+self.sub_length,self.data_len)))
                        self.sub_data = self.all_data.select(sub_range)
                        print('self.sub_data = ',self.sub_data)
                        self._setup_data()
