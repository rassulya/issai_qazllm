...
from torchtune.datasets import issai_text_completion_dataset, issai_instruct_dataset, ISSAIInstructDataset
...
class FullFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.have_packed_data = False
        self.is_packing = False

        self.cfg = cfg
        self.dataset_cfg = cfg.dataset
        self.all_data = load_dataset(self.dataset_cfg.source,data_files=self.dataset_cfg.data_files,split=self.dataset_cfg.split).shuffle(seed=42)
        ...
        self.resource_falg = True # True origin, False back
        self.sub_data = None

        self.sub_index = 0

        self.validation_ds_kk = load_dataset(self.dataset_cfg.source, data_files=self.cfg.val_data_files_kk, split=self.dataset_cfg.split)
        self.validation_ds_ru = load_dataset(self.dataset_cfg.source, data_files=self.cfg.val_data_files_ru, split=self.dataset_cfg.split)
        self.validation_ds_en = load_dataset(self.dataset_cfg.source, data_files=self.cfg.val_data_files_en, split=self.dataset_cfg.split)
        self.validation_ds_tr = load_dataset(self.dataset_cfg.source, data_files=self.cfg.val_data_files_tr, split=self.dataset_cfg.split)



        self.pbar = None

        # if not self.train_type=='base':
        #     self.all_data = self.all_data.filter(self._clear,num_proc=64)
    ...
    def setup(self, cfg: DictConfig) -> None:
        ...
        #################### PARAMETER FREEZING (STAGE 1) ####################
        # for param in self._model.parameters():
        #     param.requires_grad = False

        # self._model.tok_embeddings.weight.requires_grad = True
        ...
        _, self.val_dataloader_en = self._setup_data_val(dataset=self.validation_ds_en)
        _, self.val_dataloader_kk = self._setup_data_val(dataset=self.validation_ds_kk)
        _, self.val_dataloader_ru = self._setup_data_val(dataset=self.validation_ds_tr)
        _, self.val_dataloader_tr = self._setup_data_val(dataset=self.validation_ds_ru)
    ...
    async def set_sub_dataloader(self
        ...
        self._sample = DistributedSampler(
            ds,
            num_replicas=world_size,
            rank=rank,
            shuffle=True,
    ...
    def _setup_data_val(self, dataset):
        """
        All data related setup happens here. Currently this recipe only supports the
        DistributedSamplers with Map-style Datasets which fit into memory. Other samplers,
        iterable datasets and streaming datasets are not supported.
        ...
        # pdb.set_trace()
        if self.train_type=='base':
            ds = issai_text_completion_dataset(data=dataset, tokenizer=self._tokenizer, column=self.dataset_cfg.column, max_seq_len=self.max_seq_len)
        else:
            ds = issai_instruct_dataset(data=dataset, 
                                        tokenizer=self._tokenizer,
                                        template=self.dataset_cfg.template, 
                                        column_map=self.dataset_cfg.column_map, 
                                        max_seq_len=self.max_seq_len,
    ...
    def evaluate(self, valid_dataloader):
        ...
        # print(epoch_loss / len(valid_dataloader))
    ...
    def sub_train(self,curr_epoch,world_size,rank):
        ...
        if self.resource_falg:
            if not self._sampler:
                raise '_sampler don`t none'
            self._sampler.set_epoch(curr_epoch)
        else:
            if not self.back_sample:
                raise 'back_sample don`t none'
            self.back_sample.set_epoch(curr_epoch)
        if not self.pbar:
            self.pbar = tqdm(total=self._steps_per_epoch, disable=not (rank == 0))
        for idx, batch in enumerate(self._dataloader if self.resource_falg else self.back_dataloader):
            if (
            ...
            # Step with optimizer
            if (sub_start_steps + idx + 1) % self._gradient_accumulation_steps == 0:
                # pdb.set_trace()
            ...
            if self.global_step % self.save_per_step==0:
            ...
            if self.global_step % 1000 == 0:
                ...
                # pdb.set_trace()
                # _, val_dataloader_kk = self._setup_data_val(dataset=val_kk)
                # _, val_dataloader_ru = self._setup_data_val(dataset=val_ru)
                # _, val_dataloader_en = self._setup_data_val(dataset=self.validation_ds_en)
                # _, val_dataloader_tr = self._setup_data_val(dataset=val_tr)

                eval_loss_kk = self.evaluate(valid_dataloader=self.val_dataloader_kk)
                eval_loss_ru = self.evaluate(valid_dataloader=self.val_dataloader_ru)
                eval_loss_en = self.evaluate(valid_dataloader=self.val_dataloader_en)
                eval_loss_tr = self.evaluate(valid_dataloader=self.val_dataloader_tr)
                # Log per-step metrics
                stop_t = time.perf_counter()
                print(f'Total time for the val is: {stop_t-start_t}')
                if (self._is_rank_zero):
                    log_dict = {
                        "validation/loss_kk": eval_loss_kk,
                        "validation/loss_ru": eval_loss_ru,
                        "validation/loss_en": eval_loss_en,
                        "validation/loss_tr": eval_loss_tr,
                        "validation/perplexity_kk": math.exp(eval_loss_kk),
                        "validation/perplexity_ru": math.exp(eval_loss_ru),
                        "validation/perplexity_en": math.exp(eval_loss_en),
                        "validation/perplexity_tr": math.exp(eval_loss_tr),
                        # "validation/n_samples":val_size,
                        # "validation/total_time":stop_t-start_t
                    }
                    if self._log_peak_memory_stats:
                        log_dict.update(utils.get_memory_stats(device=self._device))
                    self._metric_logger.log_dict(
    ...
    def train(self) -> None:
        ...
        # pdb.set_trace()
        # self.val_sampler, self.val_dataloader = self._setup_data_val()
        # pdb.set_trace()
        ...
        for curr_epoch in range(self.epochs_run, self.total_epochs):
            ...
            for j in range(_all_range_len):
                # if self._is_rank_zero:
                #     log.info(f"range_index: {j}, sub_index: {self.sub_index}")

                
                # if not self.sub_index == j:
                #     continue
